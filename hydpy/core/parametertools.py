...
broadcast input array from shape (2,) into shape (366,3)
    ...
    def _toy2values_protected(
        ...
        if self._trimmed_insufficiently and hydpy.pub.options.warntrim:
            warnings.warn(
                ...
                f"{objecttools.elementphrase(self)} have been trimmed but not its time "
                f"of year-specific values.  Using the latter without modification "
                f"might result in inconsistencies."
            )
    ...
    def toys(self) -> tuple[timetools.TOY, ...]:
        ...
        return tuple(toy for toy, _ in self._toy2values_protected)
...
`pub.timegrids.stepsize` automatically.
    ...
    def trim(self, lower=None, upper=None) -> bool:
        """Extends the usual trim logic with a warning mechanism to account for that
        trimming only affects "background values" and not the time of year / value
        pairs.
...
latter without modification might result in inconsistencies.
        ...
        >>> bottomlowwaterthreshold(_1_1_12=0.0, _1_3_12=4.0)
        >>> round_(bottomlowwaterthreshold.values[:3])
        0.0, 2.0, 3.0

        >>> with pub.options.warntrim(True), warn_later():
        ...     round_(bottomlowwaterthreshold.toy_1_3_12)
        3.0
        UserWarning: The "background values" of parameter `bottomlowwaterthreshold` of \
element `?` have been trimmed but not its time of year-specific values.  Using the \
latter without modification might result in inconsistencies.
